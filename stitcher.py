...
def centered_text(draw, font, text, x, y):
    size = draw.textsize(text, font, stroke_width=2)
    n_x = x - size[0]/2
    n_y = y - size[1]/2
    # Draw shadow
    draw.text((n_x + 5, n_y + 5), font=font, text=text, fill=(0, 0, 0), stroke_fill=(0, 0, 0), stroke_width=7)
    # Draw text
    draw.text((n_x, n_y), font=font, text=text, fill=(255, 255, 255), stroke_fill=(255, 255, 255), stroke_width=2)
